...
function customAssign(...objects) {
  ...
  let objectsAssign = Object.assign(...objects);
  for (const index in objectsAssign) {
    if (typeof objectsAssign[index] === 'number') {
      objectsAssign[index] = objectsAssign[index].toString();
    }
  }
  return objectsAssign;
...
function customObjectFromEntries(entries) {
  ...
  let newArray = entries.map(function(elem) {
    if (typeof elem[0] === 'number') {
      elem[1] = elem[0].toString();
    }
    return elem;
  });
  let obj = Object.fromEntries(newArray);
  return obj;
...
function filterObjectsByKey(arr, key) {
  ...
  let result = arr.filter(function(elem) {
    let arrayKeys = Object.keys(elem);
    let indexKey = arrayKeys.indexOf(key);
    if (indexKey === -1) {
      return false;
    }
    else {
...
function getObjectValuesSum(obj) {
  ...
  let sum = valueArr.reduce(function (accumulator, currentValue) {
